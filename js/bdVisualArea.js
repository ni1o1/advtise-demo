...
function calObservedArea(observerCenter, observerHeight, observedR, observedDirection, observedAngleRange) {
    ...
    const observerdAngle = [observedDirection - observedAngleRange, observedDirection + observedAngleRange];
...
function getCirclePosition(observedArea) {
    ...
    // 生成圆弧面sector;生成圆形：crcle
    var circlePoly = turf.sector(observedArea.observerCenter, observedArea.observedGroundR,
        observedArea.observerdAngle[0], observedArea.observerdAngle[1], options);
...
//构造函数
const buildingsShapes = function(shape, height, angle, shelter, shelterShape, building) {
...
buildingsShapes.prototype = {
    ...
    isShelter: function(bS, point) { //被这个面遮挡
        ...
        if ((bS.angle[0] < this.angle[1] - 0.1) && (bS.angle[1] > this.angle[0] + 0.1) &&
            (this.maxDistance(point) > bS.minDistance(point))) {
...
function calVisibleBuilding(visualBuildings, observedArea) { //circlePoly, 
    ...
    for (var i = 0; i < buildings.length; i++) {
        ...
        //倒带
        currentBuilding = turf.rewind(currentBuilding);
        //var minAngle = 0.0
        //var maxAngle = 0.0
        ...
        turf.coordEach(buildings[i], function(currentCoord, coordIndex) {
            ...
            //获取建筑物面的四个顶点
            var point1 = (Array.from(currentCoord));
            var point2 = (Array.from(nextCoord));
            var point3 = (Array.from(nextCoord));
            var point4 = (Array.from(currentCoord));
            point1.push(0);
            point2.push(0);
...
//计算单个面的遮挡面的遮挡面积
function calSingleShelterShape(currentShape, shelterShape, centerPoint, observerHeight) {
    /*
    ...
    */
    ...
    for (var i = 0; i < 4; i++) {
        ...
        if (i < 2) {
            ...
            //求高度 
            if ((nearCenter)) { //||(currentShape.building == shelterShape.building)
                //console.log("ads");
                var height = 1000;
            } else {
                var height = observerHeight + (shelterShape.height - observerHeight) /
                    turf.distance(shelterShape.shape[3 - i], centerPoint) * turf.distance(shape[3 - i], centerPoint);
            }
    ...
    outer:
        for (var i = 0; i < shapesGroup.length; i++) {
            ...
            for (var j = 0; j < shapesGroup.length; j++) {
                if (i == j) continue;
                if (shapesGroup[i].isShelter(shapesGroup[j], point)) { //如果被遮挡
                    ...
                    if ((shapesGroup[j].angle[0] <= shapesGroup[i].angle[0] && shapesGroup[j].angle[1] >= shapesGroup[i].angle[1])
                        //||(shapesGroup[j].building == shapesGroup[i].building)
...
//从总坐标转换为以建筑物为底面的坐标
function coordSystemConversion3To2(origin, secondCoord, currentShape) {
...
function coordSystemConversion2To3(origin, coordSystem, shape2D) {
    ...
    const coord = turf.getCoords(shape2D);
    //console.log(coord);
    const coordSet = [];
    ...
    if (coord.length == 1) {
        ...
        for (var i = 0; i < coord[0].length; i++) {
            ...
            const coordWgs = turf.getCoords([x, y]); //turf.toWgs84
